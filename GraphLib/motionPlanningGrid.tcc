...
template <class weightType>
bool MotionPlanningGrid<weightType>::ObstacleAt(const CoordinatePair<long long>& coord) const{
...
/*********/
template <class weightType>
void MotionPlanningGrid<weightType>::iterator::CalcIndexCoords(CoordinatePair<long long>& coord) const {
  /*
  down, right, up, left
  even = row
  odd  = col
  ...
  */
  ...
  coord.Set(CRow() + (GEq2 ? -1 : 1)*even,
            CCol() + (GEq2 ? -1 : 1)*!even);
}

template <class weightType>
CoordinatePair<long long> MotionPlanningGrid<weightType>::iterator::Node() const {
...
template <class weightType>
MotionPlanningGrid<weightType>::iterator::iterator() : curCoord(0,0), index(0), data(nullptr) {}
template <class weightType>
MotionPlanningGrid<weightType>::iterator::iterator(int newIndex, CoordinatePair<long long> newNode,
         const MotionPlanningGrid<weightType>* dataPtr)
    : curCoord(newNode), index(newIndex), data(dataPtr) {}

template <class weightType>
typename MotionPlanningGrid<weightType>::iterator& MotionPlanningGrid<weightType>::iterator::operator=(const iterator& that) {
...
template <class weightType>
MotionPlanningGrid<weightType>::iterator::iterator(const iterator& that) { *this = that; }

template <class weightType>
const typename MotionPlanningGrid<weightType>::iterator* MotionPlanningGrid<weightType>::iterator::operator->() const { return this; }

template <class weightType>
bool MotionPlanningGrid<weightType>::iterator::operator==(const iterator& that) const {
  return index == that.index && curCoord == that.curCoord;
}

template <class weightType>
bool MotionPlanningGrid<weightType>::iterator::operator!=(const iterator& that) const { return !(*this == that); }
...
template <class weightType>
typename MotionPlanningGrid<weightType>::iterator& MotionPlanningGrid<weightType>::iterator::operator++() {
  //if the node is outside bounds or past the max index, it is invalid
  //and we should skip past
  do ++index; while (!ValidIterator() && index < 4);
  return *this;
}
