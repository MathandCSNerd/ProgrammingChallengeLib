...
template <class type>
void MultidimensionalMatrix<type>::Set(const MatCoords& coords,
                                       const type& val) {
...
void MultidimensionalMatrix<type>::SetAll(const type& that) {
  ...
  for (size_t i = 0; i < tmp.Size(); ++i) tmp(i) = 0;

  do
    Set(tmp, that);
  while (IterateCoords(tmp, dimensions));
}

template <class type>
size_t MultidimensionalMatrix<type>::PrintRow(size_t startIndex,
                                              size_t count) const {
  using namespace std;
...
template <class type>
size_t MultidimensionalMatrix<type>::CoordsToIndex(
    const MatCoords& coords) const {
  size_t sum = 0;
